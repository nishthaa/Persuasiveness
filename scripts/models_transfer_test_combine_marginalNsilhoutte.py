...
START_SELECT = 0
NUM_SELECT_MARGINALS = 600
NUM_SELECT_SILHOUTTE = 6000
NUM_CYCLES = 1
NUM_CYCLES_TEST = 1
CLASSIFIERS = [
...
print('NUM_SELECT_MARGINALS',NUM_SELECT_MARGINALS)
print('NUM_SELECT_SILHOUTTE',NUM_SELECT_SILHOUTTE)
for _classifier in CLASSIFIERS:
    accuracies = []
    # Read the constraint file
    # constraintfile = open('/home/nimadaan/cmv/pythonwksp/src_v2/marginals/PDNUMPY_K50_KMeans_silhoutte_v2.csv')
    constraintfile = codecs.open('/home/nimadaan/cmv/pythonwksp/src_v2/marginals/CORPS3_'+str(_classifier).split('(')[0]+'.csv',encoding='utf-8',errors='ignore')
    ...
    for row in constraintreader:
        if rowsread < START_SELECT:
            continue
        if rowsread >= NUM_SELECT_MARGINALS:
            break
    ...
    for row in constraintreader:
        if rowsread < START_SELECT:
            continue
        if rowsread >= NUM_SELECT_SILHOUTTE:
            break
        constraintset.add(row[0].strip())
        rowsread += 1
