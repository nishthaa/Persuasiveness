...
pos_feats = [(word_feats(f), '1') for f in posids_train ]
neg_feats = [(word_feats(f), '0') for f in negids_train ]
...
for i in range(NUM_CYCLES):
    ...
    with open('/home/nimadaan/cmv/pythonwksp/src_v2/data/CORPS3_TrainingData_'+str(i)+'.pkl', 'wb') as datafile:
        cPickle.dump(_trainfeats_, datafile)

#Valid
print('Creating validation features')
pos_feats = [(word_feats(f), '1') for f in posids_valid ]
neg_feats = [(word_feats(f), '0') for f in negids_valid ]
...
for i in range(NUM_CYCLES):
    ...
    with open('/home/nimadaan/cmv/pythonwksp/src_v2/data/CORPS3_ValidData_'+str(i)+'.pkl', 'wb') as datafile:
        cPickle.dump(_feats_, datafile)

#Test
print('Creating testing features')
pos_feats = [(word_feats(f), '1') for f in posids_test ]
neg_feats = [(word_feats(f), '0') for f in negids_test ]
