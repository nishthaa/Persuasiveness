...
infilesilhoutte = codecs.open('/home/nimadaan/cmv/pythonwksp/src_v2/marginals/PDNUMPY_K50_KMeans_silhoutte_v2.csv',encoding='utf-8',errors='ignore')
reader = csv.reader(x.replace('\x00', '') for x in infilesilhoutte)
print('Loading all silhouttes utilities...')
sil_dict = {}
for row in reader:
    word = row[0]
    sil_dict[word] = (2.0 + float(row[1]))/4.0
for dataname in datasets:
        filename = dataname+".csv"


        outfile = open('/home/nimadaan/cmv/pythonwksp/src_v2/silhouettespecificityscores/'+filename, 'w')
        ...
        for word in sil_dict:
            ...
            score = (sil_dict[word] + 0.000000001)* (freq_target/(freq_source + 0.00000001))
